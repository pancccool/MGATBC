...
def graph_constructed_common(brain_list, data_threshold, nodal, TotalNum=125):
    '''
    introduction: This function constructs the graph data for the adjacency matrix produced by the percentage threshold.（data_threshold_pc/spearman）
    :param brain_list: A list of brain networks for which graph data needs to be built.  all: list(range(10))
    :param data_threshold: 传入shuffle之后的邻接矩阵（阈值矩阵），data_threshold_pc/spearman
    :param nodal:  Node Feature Matrix，对应于data_pc, data_spearman
    :return: cell
    '''
    graph_cell = np.array(())
...
def graph_constructed_knn(knn_graph_numpy,nodal,TotalNum=125):
    '''
    :param knn_graph_numpy:  knn_graph_numpy_pc,knn_graph_numpy_spearman
    :param nodal:  Node Feature Matrix， data_pc, data_spearman,
    :return:  List knn_graph
    '''
    graph_dicts_knn = []
    for j in range(TotalNum):
